...
const Navbar = () => {
    ...
    return (
        <>
            ...
            {/* Mobile Sidebar Overlay - Outside nav for proper fixed positioning */}
            <div
                className={`md:hidden fixed inset-0 bg-black/60 z-[60] transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                onClick={() => setIsMobileMenuOpen(false)}
            />

            {/* Mobile Sidebar - Outside nav for proper fixed positioning */}
            <div
                className={`md:hidden fixed top-0 right-0 h-screen w-64 bg-slate-950 border-l border-slate-800 z-[70] flex flex-col transform transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
            >
